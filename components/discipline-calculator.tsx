...
export function DisciplineCalculator() {
  ...
  const countedHolidayList = useMemo(
    () => monthHolidayList.filter((holiday) => !isWeekendDate(holiday.date)),
    [monthHolidayList],
  );
  ...
  const statCards = [
    {
      label: "Total Shift",
      value: `${totalShift} hari`,
      detail: "Hasil dari total hari bulan dikurangi Sabtu/Minggu dan libur nasional hari kerja.",
    },
    {
      label: "Minimum 85%",
      value: `${minimumDisciplineTarget} hari`,
      detail: "Jumlah minimum hari disiplin yang harus tercapai agar tetap berada di angka 85% atau lebih.",
    },
    {
                ...
                <CardContent className="flex flex-col gap-4 sm:gap-5">
                  ...
                  <div className="grid grid-cols-2 gap-3 sm:gap-4">
                    ...
                    <ResultPanel
                      label="Total Shift"
                      value={`${totalShift} hari`}
                      detail="Total hari dalam bulan setelah dikurangi Sabtu/Minggu dan libur nasional hari kerja."
                    />
                    <ResultPanel
                      label="Target Minimum 85%"
                      value={`${minimumDisciplineTarget} hari`}
                      detail="Ini adalah angka minimum hari disiplin yang harus tercapai."
                    />
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="flex flex-col gap-5">
                  ...
                  <div className="overflow-hidden rounded-[24px] border border-border">
                    <table className="w-full border-collapse text-left text-sm">
                      ...
                      <tbody>
                        <TableRow label="Total hari dalam bulan" value={`${daysInMonth} hari`} />
                        <TableRow label="Total Sabtu & Minggu" value={`${weekends} hari`} />
                        <TableRow
                          label="Libur nasional hari kerja"
                          value={`${nationalHolidayCount} hari`}
                        />
                        <TableRow label="Total shift efektif" value={`${totalShift} hari`} />
                        <TableRow
                          label="Target minimum 85%"
                          value={`${minimumDisciplineTarget} hari`}
                        />
                  ...
                  <div className="grid gap-3">
                    {statCards.map((item) => (
                      <div
                        key={item.label}
                        className="rounded-[22px] border border-border bg-background/80 px-4 py-4"
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div>
                          ...
                          <Minus className="mt-1 size-4 text-muted-foreground" />
                        </div>
